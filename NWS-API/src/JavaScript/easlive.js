...
let global_headers3 = {'User-Agent': 'AtmosphericX','Accept': 'application/geo+json','Accept-Language': 'en-US'}
let count_settings3 = {
    latest_string : '',
    latest_int : 0,
    total_warnings: 0,
...
function decyph_event3(event_table) {  let eventType = event_table.properties.event; let eventDesc = event_table.properties.description.toLowerCase(); let tornadoThreat = event_table.properties.parameters.tornadoDetection; let thunderstorm = event_table.properties.parameters.thunderstormDamageThreat; if (eventDesc.includes(`particularly dangerous situation`)) { return `PDS Warning`; } else if (eventDesc.includes(`tornado emergency`) && eventType == `Tornado Warning`) { return `Tornado Emergency`; } else if (eventDesc == `Tornado Warning`) {  if (tornadoThreat == `RADAR INDICATED`) { return `Radar Indiciated Tornado Warning`; } else if (eventDesc == `OBSERVED`) { return `Confirmed Tornado Warning`; } else { return `Expired Tornado Warning`; } } else if (eventType == `Severe Thunderstorm Warning`) { if (thunderstorm == `DESTRUCTIVE`) { return `Destructive Severe Thunderstorm Warning`; } else if (thunderstorm == `CONSIDERABLE`) { return `Considerable Severe Thunderstorm Warning`; } else { return `Severe Thunderstorm Warning`; } }else{ return eventType; }}
function format_state3(locations) {  for (let i = 0; i < locations.length; i++) {if (locations[i] == ',') {state = locations.substring(i + 2, i + 4);return state;}}return 'NotInStates';}
function is_new_alert3(alert_temp) { if (alert_temp == count_settings3.latest_string) { return false; }else{return true;}}
function is_valid_alert3(alert_temp) {for (let i = 0; i < whitelisted_events3.length; i++) { if (alert_temp == whitelisted_events3[i]) { return true;}}return false;}
function format_locations3(locations) { following_areas = locations.split(','); if (following_areas.length > 1) { return `${following_areas}`;}return following_areas[0];}
function whithin_time_frame3(time_end) { let time_epoch = new Date(time_end).getTime();let current_time = new Date().getTime();if (time_epoch > current_time) { return true;}return false;}
function reset_active_alerts3() {  console.log("Resetting active alerts"); count_settings3.total_warnings = 0; count_settings3.total_watches = 0; let countsettings = Object.keys(count_settings3.event_counts); for (let i = 0; i < countsettings.length; i++) { count_settings3.event_counts[countsettings[i]] = 0; } }

function request_active_alerts3() {
    fetch(`https://api.weather.gov/alerts`, {headers: global_headers3}).then(response => response.text()).then(data => {
        reset_active_alerts3()
        ...
        for (let i = 0; i < alerts.length; i++) {
            let eventValid = is_valid_alert3(alerts[i].properties.event)
            let eventExpire = whithin_time_frame3(alerts[i].properties.expires)
            ...
            if (time_start_time > count_settings3.latest_int) {
                ...
                lat_text = "<p><i>Latest Response - " + messageType + "</i><br>" + eventType + " : " + format_locations3(eventLocations) + "</p>"
                if (is_new_alert3(lat_text) == true) {
                    if (messageType == `Alert`) { messageType = `Issued`;}
                    if (messageType == `Update`) { messageType = `Updated`;}
                    let what_is_event = decyph_event3(alerts[i])
                    document.getElementById("latest_message").innerHTML = `${what_is_event} (${messageType})`
                    document.getElementById("latest_message_loco").innerHTML = `${format_locations3(eventLocations)}`
                }
            }
        }
...
request_active_alerts3()
setInterval(request_active_alerts3, 8000);